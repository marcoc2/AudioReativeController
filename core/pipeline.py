...
class ARCPipeline:
    ...
    def __init__(self, extractor, grid, scene: dict):
        self._extractor = extractor
        self._grid = grid
        self._mappings = scene.get("controls", {})

    @classmethod
    def from_yaml(cls, extractor, grid, path: str) -> "ARCPipeline":
        with open(path, "r", encoding="utf-8") as f:
            scene = yaml.safe_load(f)
        return cls(extractor, grid, scene or {})
    ...
    def query(self, t: float, frame_idx: Optional[int] = None) -> Optional[FrameSlice]:
        ...
        for name, spec in self._mappings.items():
            val = self._resolve_source(spec["source"], features, grid_vals)
            if val is None:
                continue
            val = float(val)
            if "scale" in spec:
    ...
    def _resolve_source(self, source: str, features: dict, grid_vals: dict):
        """Resolve a dotted source path: 'subbands.bass', 'stems.vocals', 'centroid'."""
