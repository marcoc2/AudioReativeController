...
The skin, seams, lighting and GPU runner come from ``core/flesh``.
...
_FS = FLESH_HEADER + """
uniform float u_time, u_saccade, u_pupil, u_blink, u_hue, u_depth;
uniform vec2 u_gaze;
...
void main(){
    Cell C = fleshCell();
    vec2 q = C.q, best = C.best, bestCell = C.cell, ge = C.ge;
    float e = C.e, dn = C.dn, cs = 2.0 / u_rows;
    ...
    if (abs(v.x) < 1.0 && dU > 0.0 && dL > 0.0){
        ...
        float ao = seamAO(eL) * (1.0 - 0.55 * exp(-max(dF, 0.0) / 0.03));
        col = skinLight(n, alb, ao, v, id, exp(-max(dF, 0.0) / 0.03));
    }
...
class EyesSystem:
    ...
    def render(self, time: float, gaze=(0.0, 0.0), saccade: int = 0, pupil: float = 0.3,
        ...
        return self._wall.draw(
            light=light, u_time=float(time), u_gaze=(float(gaze[0]), float(gaze[1])), u_depth=self.depth,
            u_saccade=float(int(saccade)), u_pupil=float(np.clip(pupil, 0.0, 1.0)),
            u_blink=float(np.clip(blink, 0.0, 1.0)), u_hue=float(hue) % 1.0)
